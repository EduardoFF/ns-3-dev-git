...
int IDSIA::ServerReceiveDataFromSocketNonBlocking(Socket &socket, char **msg, int &msg_size, const int max_data)
{
  ...
  //fprintf(stderr, "Server On received %d bytes max %d bytes from socket %d\n", msg_size,
  //	  max_data, socket.sock);
  if (msg_size <= 0)
    msg_size = 0;
  else
...
{
  //  printf("ServerReceiveDataFromSocket\n");
  //fprintf(stderr, "Server On receive max %d bytes from socket %d\n", max_data, socket.sock);
  ...
  //fprintf(stdout, "Received %d bytes from socket %d\n", msg_size, socket.sock);
  return msg_size;
}
...
/** delay is only necessary when sending a packet, not for reception */
void IDSIA::GenerateMsg(char **msg, int &msg_size, const int from, const int to,
			const string content, const Real timestamp)
{
   ApplicationPacket appl;
   appl.type = APPLICATION_DATA;
   appl.from = from;
